...
"""
========================================================================================
Qlib 步进前向滚动再训练回测 (模式二：保持固定 5 年滑动窗口 / Fixed 5-Year Window)
========================================================================================

【策略与训练模式】：
1. 滚动再训练频率：【按月度滚动 (Monthly Rolling)】
2. 训练窗口模式：【固定 5 年滑动窗口 (Rolling Fixed 5-Year Window)】
   - 训练集始终保持最近固定 5 年（4 年 11 个月训练 + 刚结束的上 1 个月验证）。
   - 每个月回测开始前，吸收上个月最新数据，同时淘汰 5 年前的过旧历史数据。
   - 紧贴式切片：
     * 训练集: 预测月前 5 年 至 上上个月底
...
# ======================================================================================
# 3. 主执行流程 (Fixed 5-Year Window)
# ======================================================================================
def main():
    print("=" * 85)
    print(" 🚀 启动 Qlib 按月度滚动再训练策略 (模式二：固定 5 年滑动窗口 Fixed 5-Year Window)")
    print("=" * 85)
    ...
    print(f"       - 训练集: 预测月前 5 年至上上个月底 (固定 4 年 11 个月训练样本)")
    ...
    for i, m_start in enumerate(month_starts, 1):
        ...
        # 2. 训练集：从 预测月前 5 年 起始，滑动至上上个月底
        train_start = m_start - pd.DateOffset(years=5)
    ...
    with R.start(experiment_name="rolling_fixed_5year_workflow"):
        ...
        html_path = str(paths.reports_dir / "rolling_fixed_5year_report.html")
        ...
        png_path = str(paths.reports_dir / "rolling_fixed_5year_report.png")
        plot_and_save_static_report(report_normal_df, save_path=png_path, title_suffix="Fixed 5-Year Window")
        positions = recorder.load_object("portfolio_analysis/positions_normal_1day.pkl")
        analyze_and_save_holdings(positions, save_csv_path=str(paths.reports_dir / "rolling_fixed_5year_holdings.csv"))

    print("\n" + "=" * 85)
    print(" 🎉 固定 5 年滑动窗口滚动再训练与回测圆满完成！")
